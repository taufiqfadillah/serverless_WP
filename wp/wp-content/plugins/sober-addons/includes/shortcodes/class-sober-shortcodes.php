...
class Sober_Shortcodes {
		...
		if ( 'category' == $atts['filter'] || 'tag' == $atts['filter'] ) {
			...
			if ( ! empty( $terms ) && ! is_wp_error( $terms ) ) {
				...
				if ( 'isotope' == $atts['filter_type'] ) {
					array_unshift( $filter, '<li data-filter="*" class="line-hover active">' . esc_html__( 'All Products', 'sober-addons' ) . '</li>' );
				} elseif ( $atts['show_all'] ) {
					array_unshift( $filter, '<li data-filter="' . esc_attr( $atts[ $field ] ) . '" class="line-hover active">' . esc_html__( 'All Products', 'sober-addons' ) . '</li>' );
				}
			}
		} else {
			...
			if ( 'isotope' != $atts['filter_type'] && $atts['show_all'] ) {
				...
				$active_tab = 'recent';
				$filter[] = '<li data-filter=".recent" class="line-hover active">' . esc_html__( 'All Products', 'sober-addons' ) . '</li>';
			}
	...
	public static function post_grid( $atts ) {
		...
		while ( $posts->have_posts() ) : $posts->the_post();
			...
			$output[] = sprintf(
				...
				get_the_excerpt(),
				esc_url( get_permalink() ),
				esc_html__( 'Read More', 'sober-addons' )
			);
		endwhile;
	...
	public static function countdown( $atts ) {
		...
		$output[] = sprintf( '<div class="timer-day box"><span class="time day"></span><span class="title">%s</span></div>', esc_html__( 'Days', 'sober-addons' ) );
		$output[] = sprintf( '<div class="timer-hour box"><span class="time hour"></span><span class="title">%s</span></div>', esc_html__( 'Hours', 'sober-addons' ) );
		$output[] = sprintf( '<div class="timer-min box"><span class="time min"></span><span class="title">%s</span></div>', esc_html__( 'Mins', 'sober-addons' ) );
		$output[] = sprintf( '<div class="timer-secs box"><span class="time secs"></span><span class="title">%s</span></div>', esc_html__( 'Sec', 'sober-addons' ) );
		$output[] = '</div>';
	...
	public static function product( $atts, $content ) {
		...
		$button = sprintf(
			...
			$link['title'] ? esc_html( $link['title'] ) : esc_html__( 'Add to cart', 'sober-addons' )
		);
		$over_link = sprintf(
			...
			esc_attr( $link['target'] ),
			esc_attr( $link['rel'] ),
			esc_html__( 'View Product', 'sober-addons' )
		);
	...
	public static function banner4( $atts, $content ) {
		...
		$link = empty( $link['url'] ) ? '' : sprintf( '<a href="%s" target="%s" rel="%s" title="%s">%s</a>',
			...
			esc_attr( $link['rel'] ),
			esc_attr( $link['title'] ),
			esc_html__( 'View detail', 'sober-addons' ) );
	...
	public static function icon_box( $atts, $content ) {
		$atts = shortcode_atts( array(
			...
			'image_url'        => '',
			'style'            => 'normal',
			'title'            => esc_html__( 'I am Icon Box', 'sober-addons' ),
			'css_animation'    => '',
			'el_class'         => '',
		), $atts, 'sober_' . __FUNCTION__ );
	...
	public static function pricing_table( $atts ) {
		$atts = shortcode_atts( array(
			...
			'price'         => '',
			'currency'      => '$',
			'recurrence'    => esc_html__( 'Per Month', 'sober-addons' ),
			'features'      => '',
			'button_text'   => esc_html__( 'Get Started', 'sober-addons' ),
			'button_link'   => '',
			'color'         => '#6dcff6',
			'css_animation' => '',
			'el_class'      => '',
	...
	public static function map( $atts, $content ) {
		...
		if ( empty( $atts['api_key'] ) ) {
			return esc_html__( 'Google map requires API Key in order to work.', 'sober-addons' );
		}

		if ( empty( $atts['address'] ) && empty( $atts['lat'] ) && empty( $atts['lng'] ) ) {
			return esc_html__( 'No address', 'sober-addons' );
		}
	...
	public static function faq( $atts, $content ) {
		$atts = shortcode_atts( array(
			'title'         => esc_html__( 'Question content goes here', 'sober-addons' ),
			'open'          => 'false',
			'css_animation' => '',
			'el_class'      => '',
		), $atts, 'sober_' . __FUNCTION__ );
		...
		return sprintf(
			...
			esc_attr( implode( ' ', $css_class ) ),
			esc_html__( 'Question', 'sober-addons' ),
			esc_html( $atts['title'] ),
			esc_html__( 'Answer', 'sober-addons' ),
			$content
		);
	}
	...
	public static function get_coordinates_from_google( $address, $key ) {
		...
		if ( is_wp_error( $response ) ) {
			$results['error'] = esc_html__( 'Can not connect to Google Maps APIs', 'sober-addons' );
		...
		if ( $response['response']['code'] == 200 ) {
			...
			if ( $data->status === 'OK' ) {
				...
				$results['error'] = esc_html__( 'No location found for the entered address.', 'sober-addons' );
			} elseif ( $data->status === 'INVALID_REQUEST' ) {
				$results['error'] = esc_html__( 'Invalid request. Did you enter an address?', 'sober-addons' );
			} else {
				$results['error'] = $data->error_message;
			}
		} else {
			$results['error'] = esc_html__( 'Unable to contact Google API service.', 'sober-addons' );
		}
	...
	public static function get_coordinates_from_openstreetmap( $address ) {
		...
		if ( is_wp_error( $response ) ) {
			$results['error'] = esc_html__( 'Can not get the coordinates data', 'sober-addons' );
		...
		if ( isset( $atts['load_more'] ) && $atts['load_more'] && $products->max_num_pages > 1 ) {
			...
			if ( $paged < $products->max_num_pages ) {
				$button = sprintf(
					...
					esc_attr( wp_create_nonce( 'sober_get_products' ) ),
					esc_html__( 'Load More', 'sober-addons' )
				);
	...
	protected static function products_shortcode( $atts, $return = 'content' ) {
		if ( ! class_exists( 'WC_Shortcode_Products' ) ) {
			return __( 'Please install or update WooCommerce.', 'sober-addons' );
		}
