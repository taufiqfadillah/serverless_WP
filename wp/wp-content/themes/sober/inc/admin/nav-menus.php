...
class Sober_Mega_Menu_Edit {
	...
	public function __construct() {
		$this->modals = apply_filters(
			'sober_mega_menu_modals',
			array(
				'menus',
				'title',
				'mega',
	...
	/**
	 * Add mega menu settings link on menu item
	 *
	...
	public function add_settings_link( $item_id ) {
		...
		<fieldset class="field-menu-settings hide-if-no-js description-wide">
			...
			<span class="hidden mega-data" data-mega="<?php echo esc_attr( wp_json_encode( $mega_data ) ); ?>" aria-hidden="true">
				<?php
				// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				echo trim( $mega_content );
				?>
			</span>
	...
	public function scripts( $hook ) {
		...
		wp_register_style(
			'sober-mega-menu-admin',
			get_template_directory_uri() . '/css/admin/mega-menu.css',
			array(
				'media-views',
				'wp-color-picker',
				'font-awesome',
		...
		wp_register_script(
			'sober-mega-menu-admin',
			get_template_directory_uri() . '/js/admin/mega-menu.js',
			array(
				'jquery',
				'jquery-ui-resizable',
				'wp-util',
				...
				<div class="media-modal-content">
					...
					<div class="smm-frame-toolbar media-frame-toolbar">
						<div class="smm-toolbar media-toolbar">
							...
							<?php wp_nonce_field( 'sober-mega-menu-save', '_smm_nonce' ); ?>
						</div>
					</div>
				</div>
			</div>
	...
	public function save_menu_item_data() {
		check_ajax_referer( 'sober-mega-menu-save', 'nonce' );

		if ( ! current_user_can( 'edit_theme_options' ) ) {
			wp_send_json_error( __( 'You are not allowed to perform this action.', 'sober' ) );
		}

		if ( empty( $_POST['data'] ) ) {
			wp_send_json_error( __( 'No data found', 'sober' ) );
		}

		// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		$raw = stripslashes_deep( wp_unslash( $_POST['data'] ) );
		parse_str( $raw, $data );

		if ( empty( $data ) || empty( $data['menu-item-mega'] ) ) {
			wp_send_json_error( __( 'Invalid data', 'sober' ) );
		}
