...
function sober_body_classes( $classes ) {
	...
	if ( sober_has_page_header() ) {
		...
		if ( is_page_template( 'templates/full-screen.php' ) ) {
			...
			$page_header_bg    = get_post_meta( $page_id, 'site_header_bg', true );
			$header_bg         = $page_header_bg ? $page_header_bg : $header_bg;
			$header_bg         = 'transparent' != $header_bg ? $header_bg : ( is_page() ? $header_bg : 'white' );
			$page_header_color = get_post_meta( $page_id, 'site_header_text_color', true );
			$header_color      = in_array( $header_bg, array( 'transparent', 'custom' ) ) ? $page_header_color : $default_header_color;
		}
	...
	if ( function_exists( 'is_checkout' ) && is_checkout() ) {
		$classes[] = 'woocommerce-checkout--' . sober_get_option( 'checkout_layout' );
	}
...
function sober_customize_css() {
	...
	// Page header image.
	$page_header_image = sober_get_page_header_image();

	if ( $page_header_image ) {
		$css .= '.page-header { background-image: url(' . esc_url( $page_header_image ) . '); }';
	}
	...
	if ( is_page() && ! is_page_template( 'templates/homepage.php' ) ) {
		...
		} elseif ( 'custom' == $top_spacing ) {
			$top = get_post_meta( get_the_ID(), 'top_padding', true );
		...
		} elseif ( 'custom' == $bottom_spacing ) {
			$bottom = get_post_meta( get_the_ID(), 'bottom_padding', true );
	...
	// Product badges.
	$badge_sale_bg = sober_get_option( 'product_badge_sale_bg' );
	if ( $badge_sale_bg ) {
		$css .= '.woocommerce .ribbons .onsale {background-color: ' . $badge_sale_bg . '}';
	}

	$badge_new_bg = sober_get_option( 'product_badge_new_bg' );
	if ( $badge_new_bg ) {
		$css .= '.woocommerce .ribbons .newness {background-color: ' . $badge_new_bg . '}';
	}

	$badge_featured_bg = sober_get_option( 'product_badge_featured_bg' );
	if ( $badge_featured_bg ) {
		$css .= '.woocommerce .ribbons .featured {background-color: ' . $badge_featured_bg . '}';
	}

	$badge_soldout_bg = sober_get_option( 'product_badge_soldout_bg' );
	if ( $badge_soldout_bg ) {
		$css .= '.woocommerce .ribbons .sold-out {background-color: ' . $badge_soldout_bg . '}';
	}
...
function sober_open_content_container() {
	...
	echo '<div class="site-content-container ' . esc_attr( $class ) . '">';
...
function sober_maintenance_redirect() {
	...
	if ( current_user_can( 'super admin' ) || current_user_can( 'manage_options' ) ) {
		return;
	}
	...
	if ( get_option( 'sober_portfolio' ) && get_option( 'sober_portfolio_page_id' ) == $page_id ) {
		$is_page = $is_page || is_post_type_archive( 'portfolio' );
	}

	if ( class_exists( 'WooCommerce' ) && wc_get_page_id( 'shop' ) == $page_id ) {
...
/**
 * Support Polylang search URL.
 *
 * @param string $url Original search URL.
 * @return string
 */
function sober_search_form_action_url( $url ) {
	global $polylang;

	if ( $polylang && $polylang->curlang ) {
...
add_filter( 'sober_search_form_action', 'sober_search_form_action_url' );
